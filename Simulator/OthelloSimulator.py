...
class OthelloSimulator(GS.GameSimulator):
    def __init__(self, gammeCommand, vectorCardinality, mmTreeDepth, numPlaysPerOpponent, unratedGainInfluenceFactor = 35, gainInfluenceFactor=20, gainEstimationFactor=0.0065, defaultStrengthValue=2000):
        super(OthelloSimulator, self).__init__(gammeCommand, vectorCardinality, mmTreeDepth, numPlaysPerOpponent)
        
        self._logger = logging.getLogger('OptimizationManager.OthelloSimulator')        
    ...
    def __setstate__(self, d):
        logger = logging.getLogger('OptimizationManager.OthelloSimulator')
        d['_logger'] = logger
    ...
    def generatePairsofHeuristics(self,numberOfPairs,mode='train'):
        ...
        if mode == 'accuracy':
            ...
            for i in xrange(numberOfPairs):
                ...
                s = self._simulate(player1, player2) # simulate a game 
                result = self._calcIndividualGains(s) 
                while(int(result[0][0]) == 0):
                    s = self._simulate(self.generateOpponents(numberOfPairs), self.generateOpponents(numberOfPairs)) # simulate a game 
                    result = self._calcIndividualGains(s)'''
