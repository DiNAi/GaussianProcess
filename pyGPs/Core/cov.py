...
class CovPref(Kernel):
    ''' Covariance function for preference Learning
    Input : an Instance of pyGPs Covariance Function
    
    Output kernel is
    C([x1, x2], [x1', x2']) = K(x1, x1') + K(x2, x2') - K(x1, x2') - K(x1', x2).'''
    def __init__(self,fCov):
        self.fCov = fCov
        if self.fCov.hyp:
            self._hyp = self.fCov.hyp 
        else:
            self._hyp = []
    ...
    def getCovMatrix ( self , x = None , z = None , mode = None ):
        ...
        if mode == 'self_test':
            z1 , z2 = self._calcBlockMatrices(z)
            A = self.fCov.getCovMatrix(z=z1,mode='self_test') + self.fCov.getCovMatrix(z=z2,mode='self_test') 
            diagonal_matrix = self.fCov.getCovMatrix(x=z1,z=z2,mode='cross') + self.fCov.getCovMatrix(x=z2,z=z1,mode='cross')
            diagonal_matrix = diagonal_matrix.diagonal()
            diagonal_matrix = diagonal_matrix.reshape((z.shape[0],1))
            A = A - diagonal_matrix
            
        if mode == 'train':
            x1 , x2 = self._calcBlockMatrices(x)
            A = self.fCov.getCovMatrix ( x = x1 , mode = 'train' ) + self.fCov.getCovMatrix ( x = x2 , mode = 'train' ) - self.fCov.getCovMatrix( x = x1, z = x2, mode = 'cross' ) - self.fCov.getCovMatrix( x = x2, z = x1, mode = 'cross' ) 
            
        if mode == 'cross':
            x1 , x2 = self._calcBlockMatrices(x)
            z1 , z2 = self._calcBlockMatrices(z)
            A = self.fCov.getCovMatrix ( x = x1, z = z1, mode = 'cross' ) + self.fCov.getCovMatrix ( x = x2, z = z2, mode = 'cross' ) - self.fCov.getCovMatrix( x = x1, z = z2, mode = 'cross' ) - ( self.fCov.getCovMatrix( x = x2, z = z1, mode = 'cross' ) )
    ...
    def getDerMatrix(self , x = None , z = None , mode = None , der = None):
        ...
        if mode == 'self_test':
            z1 , z2 = self._calcBlockMatrices(z)
            A = self.fCov.getDerMatrix(z=z1,mode='self_test', der = self.der_val) + self.fCov.getDerMatrix(z=z2,mode='self_test', der = self.der_val) 
            diagonal_matrix = self.fCov.getDerMatrix(x=z1,z=z2,mode='cross', der = self.der_val) + self.fCov.getDerMatrix(x=z2,z=z1,mode='cross', der = self.der_val)
            diagonal_matrix = diagonal_matrix.diagonal()
            diagonal_matrix = diagonal_matrix.reshape((z.shape[0],1))
            A = A - diagonal_matrix
        ...
        if mode == 'train':
            x1 , x2 = self._calcBlockMatrices(x)
            A = self.fCov.getDerMatrix ( x = x1 , mode = 'train' , der = self.der_val ) + self.fCov.getDerMatrix ( x = x2 , mode = 'train' , der = self.der_val ) - self.fCov.getDerMatrix ( x = x1, z = x2, mode = 'cross' , der = self.der_val ) - self.fCov.getDerMatrix ( x = x2, z = x1, mode = 'cross' , der = self.der_val )
            
        if mode == 'cross':
            x1 , x2 = self._calcBlockMatrices(x)
            z1 , z2 = self._calcBlockMatrices(z)
            A = self.fCov.getDerMatrix ( x = x1, z = z1, mode = 'cross' , der = self.der_val ) + self.fCov.getDerMatrix ( x = x2, z = z2, mode = 'cross' , der = self.der_val ) - self.fCov.getDerMatrix( x = x1, z = z2, mode = 'cross' , der = self.der_val ) - self.fCov.getDerMatrix( x = x2, z = z1, mode = 'cross' , der = self.der_val )
            
        return A
    
    #Calculates the block matrices since preference kernel works with 2 input vectors
    def _calcBlockMatrices(self,x):
        dimension = np.shape(x)
        if dimension[1] % 2 != 0:
            raise Exception("Error: Each input vector should have even number of dimensions or columns")
        self.__vectorCardinality = int(dimension[1] / 2)
