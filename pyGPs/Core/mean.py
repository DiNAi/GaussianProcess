...
import numpy as np
import math
from bokeh.enums import Dimension
...
class MeanPref(Mean):
    '''Mean for Preference Learning.'''
    def __init__(self,fMean):
        self.fMean = fMean
        if self.fMean.hyp:
            self._hyp = self.fMean.hyp 
        else:
            self._hyp = []
    ...
    def getMean(self, x=None):
        ...
        A = self.fMean.getMean(x1) - self.fMean.getMean(x2)
        return A

    def getDerMatrix(self, x=None, der=None):
        x1 , x2 = self._calcBlockMatrices(x)
        A = self.fMean.getDerMatrix( x1 , der ) - self.fMean.getDerMatrix( x2 , der )
        return A
